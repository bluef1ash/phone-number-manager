(function ($) {
    ...
    $.learunindex = {
        ...
        loadMenu: function () {
			var _html = "";
        	$.ajax({
        		"url" : base_path + "index/getmenu.action",
        		"type" : "get",
        		"async" : false,
        		"cache" : false,
        		"data" : {"isDisplay" : 1},
        		"success" : function (data, status) {
        			if (status == "success" && data != null) {
        	            ...
        	            for (var i = 0; i < userPrivileges.length; i++) {
        	            	...
        	            	_html += '"><a href="javascript:void();"><i class="' + userPrivileges[i].iconName + '"></i><span>' + userPrivileges[i].privilegeName + '</span><i class="fa fa-angle-left pull-right"></i></a>';
        	            	if (userPrivileges[i].subUserPrivileges != null) {
        	            		_html += '<ul class="treeview-menu">';
        	            		for (var j = 0; j < userPrivileges[i].subUserPrivileges.length; j++) {
        	            			_html += '<li><a class="menuItem" href="' + base_path + userPrivileges[i].subUserPrivileges[j].uri.substring(1) + '"><i class="' + userPrivileges[i].subUserPrivileges[j].iconName + '"></i>' + userPrivileges[i].subUserPrivileges[j].privilegeName + '</a></li>';
        	            		}
        	            		_html += '</ul>';
        	            	}
        	            	_html += '</li>';
