...
public interface UserPrivilegeMapper extends CommonMapper<UserPrivilege> {
     ...
     *
     * @param display          是否在菜单中显示
     * @param constraintAuthes 系统应用集合
     * @return 系统用户权限列表
     * @throws DataAccessException 数据库操作异常
     */
    Set<UserPrivilege> selectByDisplayAndConstraintAuth(@Param("display") Boolean display, @Param("constraintAuthes") Set<String> constraintAuthes) throws DataAccessException;
