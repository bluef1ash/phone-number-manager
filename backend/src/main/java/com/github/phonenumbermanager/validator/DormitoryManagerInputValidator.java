 ...
 * @author 廿二月的天
 */
public class DormitoryManagerInputValidator extends BaseInputValidator<DormitoryManager> {
    ...
    protected boolean checkInput(Object target, Errors errors) {
        ...
        ValidationUtils.rejectIfEmpty(errors, "politicalStatus", "dormitoryManager.politicalStatus.required",
            "请选择社区楼长的政治面貌！");
        ValidationUtils.rejectIfEmpty(errors, "employmentStatus", "dormitoryManager.employmentStatus.required",
            "请选择社区楼长的工作状况！");
        ...
        ValidationUtils.rejectIfEmpty(errors, "managerAddress", "dormitoryManager.managerAddress.required",
            "社区楼长的分包楼栋不能为空！");
        ValidationUtils.rejectIfEmpty(errors, "managerCount", "dormitoryManager.managerCount.required",
            "社区楼长的联系户数不能为空！");
        ValidationUtils.rejectIfEmpty(errors, "communityId", "dormitoryManager.communityId.required", "请选择所属社区！");
        ValidationUtils.rejectIfEmpty(errors, "subcontractorId", "dormitoryManager.subcontractorId.required",
            "请选择社区分包人！");
        ...
        try {
            List<DormitoryManager> isNameAndAddressRepeat =
                dormitoryManagerService.get(nameAddress, dormitoryManager.getId(), subdistrictId);
            if (!checkedListData(isNameAndAddressRepeat, false, null)) {
                return false;
            }
            // 联系方式重复
            List<DormitoryManager> isPhonesRepeat = dormitoryManagerService.get(dormitoryManager.getPhoneNumbers(),
                dormitoryManager.getId(), subdistrictId, PhoneNumberSourceTypeEnum.DORMITORY_MANAGER);
     ...
     * @return 是否验证成功
     */
    private boolean checkedListData(List<DormitoryManager> isNameAndAddressRepeat, boolean isCheckedPhone,
        ...
        if (isNameAndAddressRepeat.size() > 0) {
            ...
            if (isCheckedPhone) {
                ...
                for (int i = 0; i < isNameAndAddressRepeat.size(); i++) {
                    if ("PUT".equals(request.getMethod()) && dormitoryManager.getId() != null) {
                        for (PhoneNumber phoneNumber : dormitoryManager.getPhoneNumbers()) {
                            if (phoneNumber.getPhoneNumber() != null
                                && StringUtils.isNotEmpty(phoneNumber.getPhoneNumber())
                                && isNameAndAddressRepeat.toString().contains(phoneNumber.getPhoneNumber())) {
                    ...
                    communityNames.append(isNameAndAddressRepeat.get(i).getCommunity().getName()).append("的")
                        .append(isNameAndAddressRepeat.get(i).getName()).append("楼长");
                    isChecked = false;
                    checkType = "联系方式";
                    field = "telephones";
                }
