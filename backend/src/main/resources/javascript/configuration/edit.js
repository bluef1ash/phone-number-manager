...
$(document).ready(() => {
    ...
    new Vue({
            ...
            created () {
                this.csrfToken = this.csrf.prop("'content';            if (
                this.configuration.keyChanged === null ||
                typeof this.configuration.keyChanged === "'undefined'            ) {
        ...
        loadUsers()
        {
            if (this.configuration.type === 4 && this.users.length === 0) {
                ...
                $ajax(
                    ...
                    (data) => {
                        this.users = data.systemUsers.map((item) => ({
                            id: item.id,
                            name: item.username
                        }))
                    },
