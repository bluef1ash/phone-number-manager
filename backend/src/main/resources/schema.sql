...
CREATE TABLE pm_company (
    ...
    actual_number MEDIUMINT(8) UNSIGNED         DEFAULT 0 COMMENT '辖区内人数',
...
CREATE UNIQUE INDEX uk_username ON pm_system_user(username);
...
CREATE TABLE pm_company_permission (
    ...
    privilege_id BIGINT UNSIGNED      NOT NULL COMMENT '系统权限所属编号',
    create_time  DATETIME             NOT NULL COMMENT '增加时间',
    update_time  DATETIME             NOT NULL COMMENT '更新时间',
    version      SMALLINT(5) UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁',
...
CREATE TABLE pm_system_permission (
    ...
    description VARCHAR(255)         NOT NULL DEFAULT '' COMMENT '系统用户权限约束描述',
...
CREATE UNIQUE INDEX uk_uri_method ON pm_system_permission(uri, method);
...
DROP TABLE IF EXISTS pm_system_user_phone_number;
CREATE TABLE pm_system_user_phone_number (
    system_user_id  BIGINT UNSIGNED      NOT NULL COMMENT '所属系统用户编号',
    phone_number_id BIGINT UNSIGNED      NOT NULL COMMENT '所属联系方式编号',
    create_time     DATETIME             NOT NULL COMMENT '增加时间',
    update_time     DATETIME             NOT NULL COMMENT '更新时间',
    version         SMALLINT(5) UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁',
    PRIMARY KEY (system_user_id, phone_number_id)
) COMMENT = '系统用户与联系方式中间表';
