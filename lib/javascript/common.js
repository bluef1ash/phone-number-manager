export default {
    ...
    deleteObject(vueObj, url, id, idField = "id") {
        if (id !== null && id !== "") {
            let csrf = $("meta[name='X-CSRF-TOKEN']");
            vueObj.$confirm("是否确定要删除此数据？", "警告", {
                ...
                this.$ajax({
                    ...
                    method: "delete",
                    headers: {
                        "X-CSRF-TOKEN": csrf.prop("content")
                    },
                    data: {
                        [idField]: id
                    }
     ...
     * @param catches
     */
    $ajax(options, then, catches = null) {
        if (catches === null) {
        ...
        $.ajax(options).then((result, status, xhr) => then && then(result, status, xhr)).catch((xhr, status, error) => catches && catches(xhr, status, error)).always((xhr, status, error) => complete && complete(xhr, status, error));
    }
};
