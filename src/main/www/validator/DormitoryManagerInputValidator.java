...
public class DormitoryManagerInputValidator extends BaseInputValidator<DormitoryManager> implements Validator {
    ...
    protected boolean checkInput(Object target, Errors errors) {
        ...
        field = "telephone";
        if (StringUtils.isEmpty(dormitoryManager.getTelephone()) && StringUtils.isEmpty(dormitoryManager.getLandline())) {
        ...
        if (StringUtils.isNotEmpty(dormitoryManager.getTelephone())) {
            phones.add(CommonUtil.qj2bj(CommonUtil.replaceBlank(dormitoryManager.getTelephone())));
        }
        if (StringUtils.isNotEmpty(dormitoryManager.getLandline())) {
            phones.add(CommonUtil.qj2bj(CommonUtil.replaceBlank(dormitoryManager.getLandline()).replaceAll("—", "-")));
        }
        field = "telephones";
        Long subdistrictId = Long.valueOf(request.getParameter("subdistrictId"));
        try {
            List<DormitoryManager> isNameAndAddressRepeat = dormitoryManagerService.findByNameAndAddress(nameAddress, dormitoryManager.getId(), subdistrictId);
            if (!checkedListData(isNameAndAddressRepeat, false, null)) {
                return false;
            }
            // 联系方式重复
            List<DormitoryManager> isPhonesRepeat = dormitoryManagerService.findByPhones(phones, dormitoryManager.getId(), subdistrictId);
            ...
            if (isCheckedPhone) {
                for (int i = 0; i < isNameAndAddressRepeat.size(); i++) {
                    if ("PUT".equals(request.getMethod()) && dormitoryManager.getId() != null) {
                        boolean isPhone1 = StringUtils.isNotEmpty(dormitoryManager.getTelephone()) && isNameAndAddressRepeat.toString().contains(dormitoryManager.getTelephone());
                    ...
                    communityNames.append(isNameAndAddressRepeat.get(i).getCommunity().getCommunityName()).append("的").append(isNameAndAddressRepeat.get(i).getName()).append("楼长");
                    isChecked = false;
                    checkType = "联系方式";
                    field = "telephones";
                }
