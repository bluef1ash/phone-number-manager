 ...
 */
            ...
            while ((cur = cur.$parent)) {
                ...
                if (hooks) {
                    for (var i = 0; i < hooks.length; i++) {
                        try {
                            var capture = hooks[i].call(cur, err, vm, info) ===
                                false;
                            if (capture) { return; }
                        } catch (e) {
    ...
    function _traverse(val, seen) {
        ...
        if ((!isA && !isObject(val)) || Object.isFrozen(val) || val instanceof
            VNode) {
            return;
        }
        if (val.__ob__) {
    ...
    function normalizeArrayChildren(children, nestedIndex) {
        ...
        for (i = 0; i < children.length; i++) {
            ...
            if (Array.isArray(c)) {
                if (c.length > 0) {
                    c = normalizeArrayChildren(c, ((nestedIndex || "") + "_" +
                        i));
    ...
    function createPatchFunction(backend) {
        ...
        function invokeDestroyHook(vnode) {
            ...
            if (isDef(data)) {
                ...
                for (i = 0; i <
                cbs.destroy.length; ++i) { cbs.destroy[i](vnode); }
            }
        ...
        function patchVnode(oldVnode, vnode, insertedVnodeQueue, removeOnly) {
            ...
            if (isDef(data) && isPatchable(vnode)) {
                for (i = 0; i <
                cbs.update.length; ++i) { cbs.update[i](oldVnode, vnode); }
            ...
            if (isUndef(vnode.text)) {
                if (isDef(oldCh) && isDef(ch)) {
                    if (oldCh !==
                        ch) { updateChildren(elm, oldCh, ch, insertedVnodeQueue, removeOnly); }
                } else if (isDef(ch)) {
                    if (isDef(oldVnode.text)) { nodeOps.setTextContent(elm, ""); }
                    addVnodes(elm, null, ch, 0, ch.length -
        ...
        function hydrate(elm, vnode, insertedVnodeQueue, inVPre) {
            ...
            if (isDef(data)) {
                if (isDef(i = data.hook) &&
                    isDef(i = i.init)) { i(vnode, true /* hydrating */); }
    ...
    function addHandler(
        ...
        if (Array.isArray(handlers)) {
            ...
            events[name] = important ? [newHandler, handlers] : [
                handlers,
                newHandler];
        } else {
    ...
    var TransitionGroup = {
        ...
        render: function render(h) {
            ...
            for (var i = 0; i < rawChildren.length; i++) {
                var c = rawChildren[i];
                if (c.tag) {
                    if (c.key != null && String(c.key).indexOf("__vlist") !==
                        0) {
                        children.push(c);
                        map[c.key] = c
        ...
        while (html) {
            ...
            if (!lastTag || !isPlainTextElement(lastTag)) {
                ...
                var reStackedTag = reCache[stackedTag] ||
                    (reCache[stackedTag] = new RegExp("([\\s\\S]*?)(</" +
                        stackedTag + "[^>]*>)", "i"));
                ...
                if (modifierCode[key]) {
                    ...
                    genModifierCode += genGuard(
                        [
                            ...
                            "shift",
                            "alt",
                            "meta"].filter(function(keyModifier) {
